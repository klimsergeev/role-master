...
**Статус:** [!] не пройден — шаблон skill-template не публикуется (корректно), но скрипт не выводит сообщение "пропущено" для директории skill-template (только для файла skill-template.md выводилось). Баг UX: отсутствует информационный вывод при пропуске директории-шаблона.
...
**TC-012: Шаблон скилла (skill-template) не публикуется**
- Severity: Low
- Основная функциональность работает: skill-template не публикуется ни в Production, ни в claude/skills/
- Проблема: скрипт не выводит информационное сообщение "пропущено" при пропуске директории skill-template (в sync_skills и sync_to_claude_skills используется `continue` без echo). Для файла skill-template.md сообщение выводится, для директории -- нет
- Рекомендация: добавить `echo` перед `continue` в блоках обработки директорий-скиллов
...
### Ручные тесты (пропущены)

- **TC-203**: URL .skill файла в роли ux-writer доступен -- требует запушенную ветку и curl
- **TC-204**: Dialog-агент ux-writer без скилла -- требует claude.ai
- **TC-205**: Dialog-агент ux-writer со скиллом -- требует claude.ai
- **TC-408**: Недоступный URL в роли -- требует claude.ai

### Найденные баги

1. **[Low] TC-012: Нет информационного вывода при пропуске директории-шаблона skill-template**
   - Компонент: publish.sh (sync_skills, sync_to_claude_skills)
   - При пропуске директории skill-template скрипт молча делает `continue`, не выводя сообщение "пропущено (шаблон)". Для файла skill-template.md сообщение выводится корректно. Несогласованность UX.
