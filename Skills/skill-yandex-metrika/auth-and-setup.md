...
## Права на счётчик ≠ OAuth-скоуп

Это две независимые оси: скоуп даёт приложению право звать API, доступ к счётчику (`own` / `view` / `edit`) даёт право читать конкретные данные.

Для Logs API достаточно `view` — владение или редактирование не требуются (проверено: токен с `view` вернул 200 и полную выгрузку). Частое ошибочное допущение «для сырых логов нужны права владельца» — неверно.
