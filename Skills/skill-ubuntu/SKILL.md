...
4. **SSH hardening не здесь** — sshd_config универсален для всех дистрибутивов и остаётся в роли devops.
...
**Запрос:** Настрой SSH так, чтобы подключаться только по ключам

**Маршрут:** -- (SSH hardening не входит в этот скилл)

**Результат:** Сообщить, что SSH hardening универсален для всех дистрибутивов Linux и покрывается ролью devops. Дать краткую подсказку: `PasswordAuthentication no` в `/etc/ssh/sshd_config`.
