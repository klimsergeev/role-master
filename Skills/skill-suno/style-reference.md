...
Конкретная тональность в Style работает как сильный дескриптор настроения и держит гармонию:

- `A minor, melancholic` — минор + настроение, связка надёжнее каждого по отдельности
- `Key: D Major, uplifting` — формат с меткой `Key:` тоже распознаётся
- Тональность ставить рядом с жанром (позиционное взвешивание)

### Прогрессии аккордов — только как ориентир

Suno НЕ следует точной прогрессии — аккорды воспринимаются как «руководство, не запрет» (как Exclude Styles). Официального механизма нет. Что работает частично:

| Приём | Где | Надёжность (оценка) |
|---|---|---|
| Тональность + настроение (`A minor, melancholic`) | Style | Высокая — на уровне жанровых дескрипторов |
| Описание прогрессии словами (`Am-F-C-G progression`, `12-bar blues`) | Style | Средняя — узнаваемые поп/блюз-формулы ловит, экзотику нет |
| Тег `[Chord progression: Am, Dm, E7, Am]` первой строкой | Lyrics | Средняя/низкая — задаёт характер гармонии, не точные аккорды |
| Аккорды в тексте БЕЗ скобок (`Am F G Em`) | Lyrics | ЗАПРЕЩЕНО — модель споёт названия аккордов вслух (lyrical contamination) |
| Аудио-референс с сыгранной прогрессией | Cover / Add Vocals + Audio Influence | Высокая — единственный почти гарантированный способ |

### Практика

- Модель тяготеет к частотным прогрессиям обучающей выборки: I-V-vi-IV, vi-IV-I-V, 12-bar blues. Назвать узнаваемую формулу — реалистично; заставить играть нестандартную последовательность текстом — нет
- Известные именованные ходы работают лучше буквенных: `12-bar blues`, `flamenco progression`, `circle of fifths` вместо перечисления аккордов
- Смена прогрессии в середине трека: второй тег `[Chord progression: ...]` перед нужной секцией. Срабатывает нерегулярно — честно предупреждать пользователя
- Для точного контроля гармонии — записать прогрессию (гитара/DAW), загрузить как референс и генерировать через Cover с высоким Audio Influence
