# Паттерны разработки Figma-плагинов

## Назначение

Переиспользуемые паттерны для типичных задач: работа с selection, обход дерева нод, batch-операции, цвета, текст, позиционирование, хранилище, обработка ошибок.

## Selection Handling

### Типизированная selection

```typescript
// Получить текстовые ноды из selection
...
async function processWithProgress<T>(
  ...
  for (let i = 0; i < total; i += batchSize) {
    ...
    // Yield — отдать управление Figma, чтобы не зависла
...
```typescript
// Figma автоматически группирует быстрые изменения в один undo step
...
// Contrast ratio (для accessibility)
...
```typescript
// Загрузить шрифт перед модификацией текста
async function setTextContent(node: TextNode, text: string): Promise<void> {
  if (node.fontName !== figma.mixed) {
    await figma.loadFontAsync(node.fontName);
  } else {
    // Mixed fonts — загрузить все уникальные шрифты
...
// Создать текстовую ноду с шрифтом
...
```typescript
// Хранение данных на ноде (переживает copy/paste)
...
// Пример: пометить обработанные ноды
...
### Валидация входных данных из UI
