...
|---|---|---|
| Первое подключение к API | [auth-and-setup.md](auth-and-setup.md) | если тестовый запрос вернул 429: [gotchas.md](gotchas.md) (справочник) |
| Создать задачу экспорта | [data-api.md](data-api.md) | если `.env` не заполнен или ответ 401: [auth-and-setup.md](auth-and-setup.md); если задача не первая за последние 10 минут, период задан в московском времени или ответ 400/429: [gotchas.md](gotchas.md) (справочник) |
| Выбрать поля для выгрузки | [data-api-fields.md](data-api-fields.md) (справочник) | если поле хранит несколько значений через запятую (`abVariant`, CD-поля): [gotchas.md](gotchas.md) (справочник) |
| Скачать и обработать CSV | [data-api.md](data-api.md), [python-reference.md](python-reference.md) | если часть скачалась битой, ответ 429 или с момента `SUCCESS` прошло больше 24 часов: [gotchas.md](gotchas.md) (справочник); если в обработке участвуют конкретные колонки (фильтр, агрегация, переименование): [data-api-fields.md](data-api-fields.md) (справочник) |
| Создать короткую ссылку | [link-manager-api.md](link-manager-api.md) | если доступ к Link Manager ещё не заведён или ответ 401: [auth-and-setup.md](auth-and-setup.md) |
| Написать Python-скрипт для выгрузки | [python-reference.md](python-reference.md), [data-api.md](data-api.md) | если скрипт создаёт больше одной задачи, качает части параллельно или фильтрует по CD-полям: [gotchas.md](gotchas.md) (справочник); если в скрипте перечисляются или фильтруются конкретные колонки CSV: [data-api-fields.md](data-api-fields.md) (справочник) |
| Отладить ошибку API | [gotchas.md](gotchas.md) (справочник) | если ошибка 401 или 403: [auth-and-setup.md](auth-and-setup.md); если 400 по параметрам запроса или неожиданный статус задачи: [data-api.md](data-api.md) |
...
Найти в таблице строку своей задачи. Всё из колонки «Обязательно» открыть до первого обращения к API. Условия колонки «Читать, если» проверить по фактам задачи -- код ответа, заполненность `.env`, период выгрузки, состав колонок -- и открыть файл там, где условие выполнено. Файлы с пометкой `(справочник)` смотреть точечно: искать подходящую строку, а не читать целиком. Задача сформулирована так, что строка таблицы не определяется, -- начать с [auth-and-setup.md](auth-and-setup.md) и уточнить постановку у пользователя, а не выбирать файлы наугад.
