...
| maxDimension | integer | Потолок длинной стороны PNG, умолчание **1024**. Поднимать, когда нужна мелкая деталь; для обзора крупной секции хватает 3000. Текстовый фрейм около 480×800 читается и на умолчании |
| enableBase64Response | boolean | Inline base64 для sandbox-окружений, умолчание **false** [R] |
...
**Результат:** По умолчанию — короткоживущий URL на скриншот плюс инструкция для curl: этот путь тратит кратно меньше токенов, чем изображение внутри ответа. Base64 приходит только при `enableBase64Response: true` — ставь флаг, когда агент не может ходить по URL (нет shell, нет HTTP-клиента, sandbox блокирует исходящие запросы). В ответе есть JSON-метаданные: `width`/`height` — размер отрисованного PNG, `original_width`/`original_height` — натуральный размер узла до ограничения. По ним видно, стоит ли перезапросить с бо́льшим `maxDimension`.
...
| Параметр | Тип | Описание |
|----------|-----|----------|
| nodeId | string | ID узла или страницы. **Необязателен:** без него инструмент возвращает не XML-дамп, а список верхнеуровневых страниц документа (guid + name) — так начинают в незнакомом файле, когда ещё неясно, куда углубляться |
| fileKey | string | Ключ файла [R] |
...
> **Выдача растёт с размером узла.** На секции 8000×4512 вернулось около 100 КБ XML — такой ответ сохраняй в файл, а не тяни в контекст. Перед вызовом на крупном узле прикинь масштаб или начни со скриншота для обзора раскладки.
