...
| **skill-agent-teams** | `Skills/skill-agent-teams.skill` | Процедура создания и управления Agent Teams в Claude Code — параллельная работа нескольких агентов с peer-to-peer коммуникацией через SendMessage. Покрывает полный цикл: создание команды (TeamCreate), спавн тиммейтов, назначение задач (TaskCreate/TaskUpdate), координация работы, завершение (shutdown_request + TeamDelete). Включает инструменты (TeamCreate, SendMessage, TeamDelete), правила для лида и тиммейтов, формат config.json, ограничения (один team на сессию, нет вложенных команд, /resume не восстанавливает). |
...
| **skill-local-permissions** | `Skills/skill-local-permissions.skill` | Процедура настройки локальных разрешений проекта в .claude/settings.json для автоматизации типовых подтверждений. Содержит пресеты (базовый, расширенный), алгоритм настройки, правила работы с permissions.allow. Скилл работает ТОЛЬКО с .claude/settings.json (уровень Local), не с глобальными или проектными settings. |
...
- **Ролей:** 23
- **Скиллов:** 27
- **Последнее обновление:** 2026-05-28 21:15
