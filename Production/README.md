...
| **skill-mssql-mcp** | `Skills/skill-mssql-mcp.skill` | Подключение Claude Code к Microsoft SQL Server через MCP-сервер: установка сервера aadversteeg/mssqlclient-mcp-server как .NET tool или Docker, регистрация через claude mcp add и .mcp.json, области действия, настройки — переменные DatabaseConfiguration, флаги EnableExecuteQuery и EnableStartQuery, MaxCellOutputLength, таймауты, Database Mode против Server Mode; передача строки подключения и пароля через .env проекта, Encrypt и TrustServerCertificate, порты, именованные экземпляры; механизмы ограничения записи, если она должна быть закрыта, — права учётной записи, гейт инструментов, permissions Claude Code; пороги усечения вывода, известные отказы и диагностика. |
...
| **skill-tsql** | `Skills/skill-tsql.skill` | Справочник языка T-SQL (диалект Microsoft SQL Server) для агентов в читающих сценариях: отличия от стандартного SQL и от PostgreSQL/MySQL — TOP вместо LIMIT, CROSS APPLY вместо LATERAL, скобки в идентификаторах, приоритет типов; обнаружение схемы через sys.* и INFORMATION_SCHEMA с официальным предупреждением; матрица «возможность → минимальная версия» и определение версии сервера и уровня совместимости базы; семантика чтения — что выглядит SELECT'ом, но меняет состояние, NOLOCK и его риски, планы без выполнения; права и видимость метаданных, границы db_datareader; DMV и типовые ловушки — даты, N'', коллации, TOP без ORDER BY. |
...
- **Ролей:** 25
- **Скиллов:** 38
- **Последнее обновление:** 2026-09-07 02:19
