...
**ВАЖНО: Выполни эти шаги ПЕРЕД любыми вызовами MCP-инструментов.**

### Чек-лист запуска

- [ ] **Figma Desktop запущен** — локальный MCP работает только с десктопным приложением
- [ ] **Файл макета открыт** — нужный файл должен быть открыт в Figma
- [ ] **MCP-сервер включён** — см. инструкцию ниже
- [ ] **Токен настроен** — в `~/.claude/settings.json` указан токен Figma (см. раздел "Настройка конфигурации")

### Как включить MCP-сервер

1. В Figma Desktop нажми `Shift+D` (переход в Dev Mode)
2. В правой панели **Inspect** найди секцию **MCP server**
3. Нажми **Enable desktop MCP server**
4. Дождись сообщения о запуске на `127.0.0.1:3845`

### Проверка работоспособности

После включения сервера выполни тестовый вызов:

```
mcp__figma__get_figma_data(fileKey: "...", nodeId: "...")
```

- **Успех:** Получены данные макета — можно работать
- **Ошибка соединения:** MCP-сервер не включён — вернись к чек-листу
- **Ошибка авторизации:** Проверь токен в конфигурации
...
| Тип | URL | Авторизация |
|-----|-----|-------------|
| **Desktop (локальный)** | `http://127.0.0.1:3845/mcp` | Personal Access Token в конфигурации |
...
1. Открой [Figma Account Settings](https://www.figma.com/settings) (или Figma -> Settings -> Account)
2. Прокрути до секции **Personal access tokens**
3. Нажми **Generate new token**
4. Скопируй токен (начинается с `figd_`)
5. Вставь в конфигурацию `~/.claude/settings.json` в поле `X-Figma-Token`
...
| Ошибка авторизации (401/403) | Токен не указан или невалиден | Проверить `X-Figma-Token` в `~/.claude/settings.json` |
