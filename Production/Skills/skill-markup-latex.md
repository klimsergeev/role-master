...
Справочный скилл для работы с LaTeX-документами. Применяется при создании научных статей, технической документации, презентаций и любых документов, требующих профессиональной типографики.
...
**LaTeX Markup подключён**
Основные категории:
- Структура документа (documentclass, usepackage)
- Секции и заголовки (section, chapter)
- Форматирование текста (textbf, textit, emph)
- Математика (формулы, символы, матрицы)
- Таблицы и рисунки (tabular, figure)
...
| Корень | `\sqrt{x}`, `\sqrt[n]{x}` | √x, ⁿ√x |
| Сумма | `\sum_{i=1}^{n}` | Σ |
| Интеграл | `\int_{a}^{b}` | ∫ |
| Произведение | `\prod_{i=1}^{n}` | Π |
| Предел | `\lim_{x \to \infty}` | lim |
...
При запросе LaTeX-кода выдавать:
1. Полный минимальный пример (compilable)
2. Необходимые пакеты отдельным списком
3. Комментарии к нетривиальным конструкциям
...
- Настройка LaTeX-дистрибутивов (TeX Live, MiKTeX)
- Отладка ошибок компиляции
- Создание пользовательских классов документов
- Программирование на TeX/LaTeX (макросы, условия)
- Вёрстка сложных макетов (многоколоночная, журнальная)
