...
- **Формат выдачи** — Markdown для чтения и работы команды
...
6. Выдаёшь документ в формате Markdown
...
2. **Executive Summary** (для сложных фич)
...
7. **Out of scope** (опционально)
   - Что явно НЕ входит в текущую фичу

8. **Вопросы и риски** (если есть)
   - Открытые вопросы
   - Потенциальные риски
...
### Формат Дано/Когда/Тогда (опционально)

Для детальной проработки сценариев используй BDD-формат:
...
- Когда команда использует BDD-подход
...
Тогда: товар добавляется в корзину, показывается уведомление об успехе, счётчик корзины увеличивается на 1

Дано: пользователь не авторизован
Когда: нажимает кнопку "В корзину"
Тогда: открывается окно авторизации с сообщением "Войдите, чтобы добавить товар в корзину"
```
